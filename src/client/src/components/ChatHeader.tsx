...
const ChatHeader: React.FC<ChatHeaderProps> = ({ tenant, isConnected, onClose, theme = 'light' }) => {
  return (
    <div
      ...
      ${tenant?.branding?.primaryColor ? `bg-[${tenant.branding.primaryColor}]` : 'bg-blue-600'}
    `}>
      <div className='flex items-center space-x-3'>
        {/* Law Firm Logo or Avatar */}
        <div className='w-10 h-10 rounded-full bg-white bg-opacity-20 flex items-center justify-center'>
          {tenant?.branding?.logo ? (
            <img src={tenant.branding.logo} alt={tenant.name} className='w-8 h-8 rounded-full object-cover' />
          ) : (
        ...
        <div>
          ...
          <div className='flex items-center space-x-1'>
            ...
            <span className='text-xs opacity-90' style={{ color: '#000000' }}>
